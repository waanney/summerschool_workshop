...
class CalculationInput(BaseModel):
    ...
    degrees: bool = Field(default=False, description="Whether to use degrees for trigonometric functions")
    precision: Optional[int] = Field(default=None, description="Number of decimal places to round to")
...
class BasicOperationInput(BaseModel):
    ...
    operation: str = Field(..., description="Operation: add, subtract, multiply, divide, power")
...
class LogarithmInput(BaseModel):
    ...
    number: float = Field(..., gt=0, description="Number to calculate logarithm of (must be positive)")
    base: float = Field(default=math.e, gt=0, description="Base of logarithm (default: e for natural log)")
...
class MemoryOperation(BaseModel):
    ...
    operation: str = Field(..., description="Memory operation: store, recall, clear, add, subtract")
    value: Optional[float] = Field(default=None, description="Value for store/add/subtract operations")
...
class CalculatorTool:
    ...
    def calculate_with_validation(self, input_data: CalculationInput) -> CalculationOutput:
        """
    ...
    def trigonometric_operation(self, input_data: TrigonometricInput) -> CalculationOutput:
        ...
        try:
            ...
            if input_data.function not in functions:
                raise ValueError(f"Unsupported trigonometric function: {input_data.function}")
            
            result = functions[input_data.function](input_data.angle, input_data.degrees)
    ...
    def memory_operation(self, input_data: MemoryOperation) -> dict:
        ...
        try:
            ...
            if input_data.operation not in operations:
                raise ValueError(f"Unsupported memory operation: {input_data.operation}")
            
            if input_data.operation in ['store', 'add', 'subtract']:
                if input_data.value is None:
                    raise ValueError(f"Value required for {input_data.operation} operation")
    ...
    def _determine_operation_type(self, expression: str) -> OperationType:
        ...
        if any(func in expression for func in ['sin', 'cos', 'tan', 'asin', 'acos', 'atan']):
    ...
    def calculate(self, expression: str, degrees: bool = False, precision: Optional[int] = None) -> Union[float, int]:
        """
...
def calculate_expression(expression: str, degrees: bool = False, precision: Optional[int] = None) -> str:
    """
