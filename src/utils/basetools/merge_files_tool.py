...
class MergeOutput(BaseModel):
    ...
    status: MergeStatus = Field(MergeStatus.FAILED, description="Status of the merge operation")
...
def merge_files_tool(input_data: MergeInput) -> MergeOutput:
    ...
    try:
        ...
        header, all_rows, rows_from_file1, rows_from_file2 = _merge_csv_files(
            input_data.file_path1, 
            input_data.file_path2,
            input_data.skip_duplicates
        )
...
def _write_merged_csv(output_path: str, header: List[str], rows: List[List[str]]) -> None:
    """
