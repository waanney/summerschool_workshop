...
class DocumentChunkingInput(BaseModel):
    ...
    language: str = Field(
        "vi", description="The language of the document ('en' or 'vi')."
    )
...
def document_chunking_tool(input: DocumentChunkingInput) -> DocumentChunkingOutput:
    ...
    try:
        ...
        if doc_path.suffix.lower() == ".txt":
            ...
            return DocumentChunkingOutput(
                success=False, message="Unsupported file type", num_chunks=0
            )

        if not content:
            return DocumentChunkingOutput(
                success=False, message="Document is empty.", num_chunks=0
            )
        ...
        if not chunks:
            return DocumentChunkingOutput(
                success=False, message="No chunks generated.", num_chunks=0
            )
        ...
        try:
            ...
            return DocumentChunkingOutput(
                success=True, message=message, num_chunks=len(chunks)
            )
        ...
        traceback.print_exc()
        return DocumentChunkingOutput(
            success=False, message=f"An error occurred: {str(e)}", num_chunks=0
        )
